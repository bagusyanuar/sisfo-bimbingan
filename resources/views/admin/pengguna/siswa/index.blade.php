    ...
    <div class="container-fluid pt-3">
        ...
        <div class="w-100 p-2">
            <div class="text-right mb-2 pr-3">
                <a href="/siswa/tambah" class="btn btn-primary"><i class="fa fa-plus mr-1"></i><span
            ...
            <table id="table-data" class="display w-100 table table-bordered">
                ...
                @foreach($data as $v)
                    <tr>
                        ...
                        <td>{{ $v->username }}</td>
                        <td>{{ $v->siswa->nama }}</td>
                        <td>{{ $v->siswa->kelas->nama }}</td>
                        <td>{{ $v->siswa->kelas->jurusan->nama }}</td>
                        <td>{{ $v->siswa->no_hp }}</td>
                        <td>{{ $v->siswa->alamat }}</td>
                        <td class="text-center">
                            <a href="/siswa/edit/{{ $v->id }}" class="btn btn-sm btn-warning btn-edit"
...
@section('js')
    ...
    <script type="text/javascript">
        function destroy(id) {
            AjaxPost('/siswa/delete', {id}, function () {
